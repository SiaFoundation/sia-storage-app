...
describe('imports ops', () => {
  ...
  it("derives summary status and per-state counts from an import's children", async () => {
    ...
    const [s] = await queryImportSummary(db(), ['i1'], 1000)
    ...
    expect(s.backoffPending).toBe(0)
    expect(s.soonestNextAttemptAt).toBeNull()
  })

  it('summary counts backoff rows and reports the soonest retry time', async () => {
    await insertImport(db(), imp({ id: 'i1', source: 'picker', sealed: 1 }))
    await insertManyImportFiles(db(), [
      file({ id: 'a', importId: 'i1', state: 'pending', attempts: 2, nextAttemptAt: 5000 }),
      file({ id: 'b', importId: 'i1', state: 'pending', attempts: 1, nextAttemptAt: 9000 }),
      // Elapsed backoff and never-attempted rows are claimable, not waiting.
      file({ id: 'c', importId: 'i1', state: 'pending', attempts: 1, nextAttemptAt: 500 }),
      file({ id: 'd', importId: 'i1', state: 'pending' }),
    ])
    const [s] = await queryImportSummary(db(), ['i1'], 1000)
    expect(s.backoffPending).toBe(2)
    expect(s.soonestNextAttemptAt).toBe(5000)
  })

  it('summary counts running copies in active, separate from pending', async () => {
    await insertImport(db(), imp({ id: 'i1', source: 'picker', sealed: 1 }))
    await insertManyImportFiles(db(), [
      file({ id: 'a', importId: 'i1', state: 'active', claimToken: 't' }),
      file({ id: 'b', importId: 'i1', state: 'pending' }),
    ])
    const [s] = await queryImportSummary(db(), ['i1'], 1000)
