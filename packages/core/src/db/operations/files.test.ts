...
describe('queryUnuploadedFileBytes', () => {
  ...
  it('excludes a file whose local bytes are gone (lost before upload)', async () => {
    await insertFile(db(), makeFileRecord('lost-1', { size: 700 }))
    await insertFile(db(), makeFileRecord('local-1', { size: 500 }))
    await seedLocalBytes('local-1', 500)
    expect(await queryUnuploadedFileBytes(db())).toBe(500)
  })

  it('returns zero when every file is uploaded', async () => {
    await insertFile(db(), makeFileRecord('uploaded-1', { size: 100 }))
    await seedLocalBytes('uploaded-1', 100)
    await insertObject(db(), makeLocalObject('uploaded-1'))
