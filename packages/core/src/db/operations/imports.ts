...
/** Automated sources (nobody is waiting on them), vs interactive picker/camera/share. */
export function isBackgroundSource(source: ImportSource): boolean {
...
export function isPaceable(source: ImportSource, sourceKind: ImportSourceKind): boolean {
  return sourceKind !== 'ephemeral' && isBackgroundSource(source)
}
...
export type ImportSummary = {
  ...
  cancelled: number
  inFlight: number
  /** Rows whose copy is running right now. `status` can't say this: an
   * unsealed import reads `importing` even while idle. */
  active: number
  total: number
  ...
  totalBytes: number
  /** Pending rows waiting out retry backoff (attempts > 0, nextAttemptAt in the future). */
  backoffPending: number
  /** Earliest nextAttemptAt among the backoff rows; null when none. */
...
export async function queryImportSummary(
  ...
  for (const r of rows) {
    ...
    if (r.state === 'pending' || r.state === 'active') {
      s.inFlight += r.n
      if (r.state === 'pending') {
        s.backoffPending += r.backoff
        if (r.soonest != null) {
          s.soonestNextAttemptAt =
            s.soonestNextAttemptAt == null ? r.soonest : Math.min(s.soonestNextAttemptAt, r.soonest)
        }
      }
