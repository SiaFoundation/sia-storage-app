...
describe('detectMimeType', () => {
  it('returns providedType when recognized', () => {
    ...
    expect(detectMimeType({ providedType: 'application/pdf' })).toBe(
      'application/pdf',
    )
  })
  ...
  it('ignores unrecognized providedType and falls through to extension', () => {
    expect(
      detectMimeType({ providedType: 'foo/bar', fileName: 'photo.jpg' }),
    ).toBe('image/jpeg')
  })

  it('falls through to extension when providedType is null', () => {
    expect(detectMimeType({ providedType: null, fileName: 'photo.png' })).toBe(
      'image/png',
    )
  })
  ...
  it('prioritizes providedType over extension', () => {
    expect(
      detectMimeType({ providedType: 'image/png', fileName: 'photo.jpg' }),
    ).toBe('image/png')
  })

  it('prioritizes extension over bytes', () => {
    const jpegBytes = new Uint8Array([0xff, 0xd8, 0xff, 0xe0])
    expect(detectMimeType({ fileName: 'image.png', bytes: jpegBytes })).toBe(
      'image/png',
    )
  })
  ...
  it('returns application/octet-stream when all signals are null', () => {
    expect(
      detectMimeType({ providedType: null, fileName: null, bytes: null }),
    ).toBe('application/octet-stream')
  })

  it('returns application/octet-stream for unknown extension and no bytes', () => {
    expect(detectMimeType({ fileName: 'file.xyz' })).toBe(
  ...
  it('falls through unrecognized bytes to octet-stream', () => {
    ...
    expect(detectMimeType({ bytes: randomBytes })).toBe(
      'application/octet-stream',
    )
  })
})
