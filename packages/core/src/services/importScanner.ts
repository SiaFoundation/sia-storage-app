...
export type ImportScannerResult = {
  ...
  deferred: number
  /**
   * The tick's deferral signals: `cause` names the binding pressure
   * (critical-floor over headroom over backlog), null when none. Byte fields
   * are null when the probe failed, and `pendingLocalBytes` also when no
   * paceable candidate existed (the backlog read is skipped then).
   */
  pressure: {
...
export class ImportScanner {
  ...
  initialize(
    ...
    readHeaderBytes: ReadHeaderBytes,
    ...
    this._readHeaderBytes = readHeaderBytes
  ...
  async runScan(signal?: AbortSignal): Promise<ImportScannerResult> {
    ...
    const result: ImportScannerResult = {
      ...
      skipped: 0,
      deferred: 0,
      pressure: { cause: null, freeBytes: null, pendingLocalBytes: null },
    }
    ...
    try {
      ...
      result.pressure = {
        ...
        pendingLocalBytes: pressure.pendingLocalBytes,
      }
        ...
        try {
            ...
            if (!copyRaced.ok) {
              ...
              if (copied.kind !== 'plain') {
                hashedInCopy = {
                  ...
                  size: copied.size,
                  headerBytes: copied.kind === 'stream' ? copied.headerBytes : undefined,
                  mediaMime: copied.kind === 'asset' ? copied.mediaMime : undefined,
                }
              }
          ...
          if (hashedInCopy) {
            ...
            outcome = {
              ...
              type: classifyImportType({
                stagedType: row.type,
                name: row.name,
                sourceKind: row.sourceKind,
                headerBytes: hashedInCopy.headerBytes,
                mediaMime: hashedInCopy.mediaMime,
              }),
            }
          } else {
            const fileUri = app.fs.uri({ id: row.id, type: row.type })
      ...
      if (result.deferred > 0) {
        ...
        logger.info('importScanner', 'paced_deferred', {
  ...
  private async readPressure(
    ...
    pacedUnderPressure: boolean
    belowCriticalFloor: boolean
    cause: 'critical-floor' | 'headroom' | 'backlog' | null
    freeBytes: number | null
    pendingLocalBytes: number | null
  }> {
    try {
      ...
      const belowCriticalFloor = freeBytes < IMPORT_CRITICAL_FREE_BYTES
      const pacedUnderPressure = hasPaceable && (storageHeadroomThin || backlogHigh)
      return {
        belowCriticalFloor,
        pacedUnderPressure,
        // The binding cause, most severe first; headroom over backlog because
        // space is the user-salient fact when both hold (both resolve as
        // uploads drain).
        cause: belowCriticalFloor
          ? 'critical-floor'
          : pacedUnderPressure
            ? storageHeadroomThin
              ? 'headroom'
            ...
            : null,
        freeBytes,
        pendingLocalBytes: hasPaceable ? pendingLocalBytes : null,
      }
    } catch (e) {
      ...
      return {
        pacedUnderPressure: false,
        belowCriticalFloor: false,
        cause: null,
        freeBytes: null,
