...
describe('ImportScanner claim-loop', () => {
  ...
  it('classifies a docx from zip header bytes plus its name instead of bare zip', async () => {
  ...
  it('header bytes correct a mislabeled row; a media row takes the OS-reported mime', async () => {
    const JPEG_HEAD = new Uint8Array([0xff, 0xd8, 0xff, 0xe0])
    const m = createMocks({
      candidates: [
        // Named and staged as png, but the bytes are jpeg: content wins.
        fileRow({
          id: 'a',
          importId: 'imp1',
          sourceKind: 'bookmark',
        ...
        fileRow({ id: 'b', importId: 'imp1', type: 'image/heic', mediaAssetId: 'm1' }),
      ],
    })
    m.fs.importCopy
    ...
    const typeById = new Map(
      m.imports.recordHash.mock.calls.map((c) => [c[0], (c[2] as { type: string }).type]),
    )
...
describe('ImportScanner critical free-space floor', () => {
  ...
  it('holds an interactive durable row that the paced gate would let through', async () => {
    ...
    expect(result.pressure.cause).toBe('critical-floor')
  })
...
describe('ImportScanner paced throttle', () => {
  ...
  it('defers a paceable row under low device space (not claimed or copied)', async () => {
    ...
    expect(result.deferred).toBe(1)
    expect(result.finalized).toBe(0)
    expect(result.pressure.cause).toBe('headroom')
  })
  ...
  it('claims, copies, and finalizes a paceable row when space is ample and the backlog is low', async () => {
    ...
    expect(result.finalized).toBe(1)
    expect(result.pressure.cause).toBeNull()
  })
  ...
  it('defers a paceable row under a high upload backlog even with ample space', async () => {
    ...
    expect(m.fs.importCopy).not.toHaveBeenCalled()
    expect(result.deferred).toBe(1)
    expect(result.pressure.cause).toBe('backlog')
    expect(result.pressure.pendingLocalBytes).toBe(600 * 1024 ** 2)
  })
