...
export function useIndexerURL() {
  const app = useApp()
  return useSWR(app.caches.settings.key('indexerURL'), () =>
    app.settings.getIndexerURL(),
  )
}
...
export function useHasOnboarded() {
  const app = useApp()
  return useSWR(app.caches.settings.key('hasOnboarded'), () =>
    app.settings.getHasOnboarded(),
  )
}
...
export function useShowAdvanced() {
  const app = useApp()
  return useSWR(app.caches.settings.key('showAdvanced'), () =>
    app.settings.getShowAdvanced(),
  )
}
...
export function useAutoScanUploads() {
  const app = useApp()
  return useSWR(app.caches.settings.key('autoScanUploads'), () =>
    app.settings.getAutoScanUploads(),
  )
}
...
export function useMaxDownloads() {
  const app = useApp()
  return useSWR(app.caches.settings.key('maxDownloads'), () =>
    app.settings.getMaxDownloads(),
  )
}
...
export function useLogLevel() {
  const app = useApp()
  return useSWR(app.caches.settings.key('logLevel'), () =>
    app.settings.getLogLevel(),
  )
}
...
export function useLogScopes() {
  const app = useApp()
  return useSWR(app.caches.settings.key('logScopes'), () =>
