...
export interface AppService {
  ...
  directories: {
    ...
    /** Returns the number of directories, without loading or counting into them. */
  ...
  fs: {
     ...
     * classification input from the copy's single read; the `plain` arm means
     * the caller hashes and classifies from the local file.
     * `signal`/`onProgress` are in-process-only and dropped over IPC; an IPC
     * transport must base64 `headerBytes`.
     */
    importCopy(
      file: { id: string; type: string },
      sourceUri: string,
     ...
     * only), since it already knows the size and never reads the hash back. */
  ...
  imports: {
     ...
     * the retry-backoff fields; defaults to the wall clock. */
    summary(ids: string[], now?: number): Promise<ImportSummary[]>
    ...
    /** Count of in-flight (pending+active) import files, optionally scoped to one destination directory. */
    countInFlight(directoryId?: string): Promise<number>
