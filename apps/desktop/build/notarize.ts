...
export async function notarize(path: string, key: NotaryKey, keyPath: string): Promise<void> {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    submitted =
      await $`xcrun notarytool submit ${path} ${auth} --wait --timeout 30m --output-format json`
        .nothrow()
        .text()
  ...
  throw new Error(`Notarization ${status ?? 'did not complete'}: ${log.trim() || submitted.trim()}`)
}
