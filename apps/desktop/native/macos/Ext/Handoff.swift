...
//
// No contents cross the socket. The kernel hands the extension a URL on the way
// up and expects one back on the way down, and both are relocated into the
// shared handoff directory by reference wherever the filesystem allows it. Only
// the fallback copies, reached when both link(2) and rename(2) fail.
...
public struct Handoff {
    ...
    public func prepare() throws {
        for sub in [SiaPaths.fetchSubdir, SiaPaths.stageSubdir] {
            let path = "\(root)/\(sub)"
            SiaPaths.ensureDirectory(path)
            // ensureDirectory swallows its error, so a file sitting at that
            // path would otherwise surface later as a missing file mid-transfer.
            ...
            guard
                ...
                throw HandoffError.unavailable("handoff directory is not writable: \(path)")
            }
        }
    }
    ...
    ///
    /// Attempts the directory rather than assuming `prepare()` has run: it runs
    /// in the background at launch, and the system can drive a fetch before it
    ...
    /// is fine because the URL the kernel hands over is a throwaway. A copy
    /// last, for the cross-volume case, which is the only path that moves bytes.
