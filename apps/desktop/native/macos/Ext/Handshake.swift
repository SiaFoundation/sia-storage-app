...
public actor Handshake {
    ...
    /// The connection epoch the current agreement belongs to. `ready` compares
    /// it inside the actor, so a callback racing the asynchronous reset after
    /// a stream drop can never be served from the old daemon's agreement.
    private var epoch = 0
    ...
    public func reset() {
        ...
        attempt = nil
        generation += 1
        // A reset means the daemon changed, so a failure recorded against the
        // old one must not rate limit the first handshake with the new one.
        lastFailureAt = nil
    }

    /// Returns once agreement holds, otherwise throws and leaves the next caller
    /// free to try again.
    public func ready(epoch: Int? = nil) async throws {
        // Actor-serialized epoch check: the caller reads its epoch from a
        // synchronous marker the disconnect handler bumps before any task
        // runs, closing the gap where the reset itself has not landed yet.
