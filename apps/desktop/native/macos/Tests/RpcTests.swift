...
final class StreamLifecycleTests: XCTestCase {
    ...
    func testAStopIsNotReportedAsADisconnect() throws {
        let path = NSTemporaryDirectory() + "sia-stop-\(getpid()).sock"
        unlink(path)
        let listener = try Self.listen(at: path)
        defer {
            close(listener)
        ...
        // Holds the connection open until the test releases it, so the read loop
        // is parked and only stop() can end it.
        ...
        let disconnected = expectation(description: "disconnect reported")
        disconnected.isInverted = true

        let stream = RpcStream(socketPath: path) { _ in }
        stream.start(onConnected: { connected.fulfill() }, onDisconnect: { _ in
