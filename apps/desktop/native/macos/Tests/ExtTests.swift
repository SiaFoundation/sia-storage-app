...
final class ErrorMappingTests: XCTestCase {
    ...
    func testTheAlertForAMissingItemDoesNotNameItsId() {
        let mapped = mapRemoteMessage("No file with id 0f9c2b41")

        XCTAssertFalse(mapped.localizedDescription.contains("0f9c2b41"))
...
final class HandshakeTests: XCTestCase {
    ...
    func testAResetClearsTheFailureBackoffSoTheNextHandshakeRunsAtOnce() async {
        let attempts = Attempts()
        let reachable = Mutable(false)
        let handshake = Handshake(retryAfter: 60, now: { 0 }) {
            attempts.bump()
            if !reachable.value { throw Unreachable() }
            return try hello()
        }

        // A failure starts the backoff; without the reset clearing it, the
        // next attempt inside retryAfter would be refused without a connect.
        try? await handshake.ready()
        reachable.value = true
...
/// A privacy annotation is a per-call decision, and the wrong one publishes a
/// container path into every sysdiagnose a user attaches to a bug report.
/// `Logger.failure` is the sanctioned way to log an error, and this catches the
/// one spelling that would undo it.
final class LogPrivacyTests: XCTestCase {
    func testNoCallSiteLogsErrorTextPublicly() throws {
        let ext = URL(fileURLWithPath: #filePath)
        ...
        for file in files where file.pathExtension == "swift" {
            ...
            XCTAssertFalse(
                source.contains("localizedDescription, privacy: .public"),
                "\(file.lastPathComponent) logs error text publicly; use fpLog.failure instead")
        }
    }
}
