...
public final class RpcStream: @unchecked Sendable {
    ...
    ///
    /// Returns immediately. Every callback runs on this object's private queue.
    /// The two callbacks are the only signal either side has that the daemon
    /// came or went, because the version handshake caches its success and stops
    /// making round trips after the first one.
    public func start(
        ...
        queue.async { [weak self] in
            ...
            do {
                ...
                self.readLoop(fd: fd)
                self.closeCurrent()
                self.report(RpcError.unreachable("change stream closed"), to: onDisconnect)
            } catch {
                // Reached with a connection already open when the subscribe
                // write fails, so the socket is released here too.
                self.closeCurrent()
                self.report(error, to: onDisconnect)
            }
        }
    }

    /// Both exits from `start()` come through here: a stop() is not a
    /// disconnect, and either one can be the way out when teardown races it.
    private func report(_ error: Error, to onDisconnect: (Error) -> Void) {
        lock.lock()
        let deliberate = stopped
        lock.unlock()
        if !deliberate { onDisconnect(error) }
    }
    ...
    public func stop() {
        ...
        // close(2) alone does not reliably return a recv already blocked here,
        // and shutdown(2) does, so the read loop ends now.
    ...
    private func readLoop(fd: Int32) {
        ...
        while true {
            ...
            while let newline = buffer.firstIndex(of: 0x0A) {
                ...
                // Anything else here answers a request the stream never sends,
