...
public final class Rpc: @unchecked Sendable {
        ...
        -> Int32
    {
        ...
        // Writing to a socket the daemon already closed raises SIGPIPE, whose
        // default disposition would take the extension down with it.
        ...
        guard bytes.count < MemoryLayout.size(ofValue: address.sun_path) else {
            close(fd)
            // The length, not the path: this message reaches log lines marked
            // public, and the path names the container and the account in it.
            throw RpcError.unreachable(
                "socket path is too long: \(bytes.count) bytes, limit "
        ...
        if result < 0 {
            close(fd)
            // The path is the same for every call this process makes, so it
            // would repeat in every connect failure the log records.
