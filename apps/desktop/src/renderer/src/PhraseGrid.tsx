...
export function PhraseGrid({ slots, onChange, readOnly, disabled }: Props) {
  ...
  function write(index: number, word: string) {
    // Both flags: a suggestion click bypasses the input element, so a grid
    // disabled mid-submit could otherwise still change the phrase.
              ...
              >
                {options.map((option, rank) => (
                  ...
                  >
                    <button
                      type="button"
                      // Out of sequential tab order, so Tab leaves the grid
                      // for the submit button; the arrows and Enter are the
                      // keyboard path to these.
                      tabIndex={-1}
