...
async function resolvePathResponse(
  ...
  if (fileName) {
    ...
    if (file) {
      ...
      // ?share is an API endpoint and ignores Accept negotiation.
      ...
      // HTML files served directly to the browser bypass the SPA shell so
      // user-authored pages render instead of the file viewer.
  ...
  if (dir) {
    // index.html short-circuits the listing — same SPA-bypass rule as files.
  ...
  // Fallthrough: path matches no file or directory directly.

  // Clean URL: serve `<name>.html` when the user requested `<name>`.
  if (fileName) {
  ...
  // SPA fallback: walk up looking for an index.html owned by the same route.
  ...
  // Browser misses still get the SPA so it can render its own 404 page.
  // API misses get a JSON 404.
  if (wantsHtml(req)) {
    sendHtml(res, UI_HTML)
    return
  }
