...
public enum BookmarkEngine {
  ...
  public static func resolve(ref: String) throws -> Resolved {
    ...
    do {
      ...
      // Only a genuinely missing target is terminal; every other resolution
      // failure (scope refusal, protected data, transient provider state)
      // must stay retryable or a live file gets marked deleted.
      let ns = error as NSError
      let missing =
        ns.domain == NSCocoaErrorDomain
        && (ns.code == NSFileNoSuchFileError || ns.code == NSFileReadNoSuchFileError)
      throw CodedError(missing ? "deleted" : "resolver-error", ns.localizedDescription)
    }
  }
