...
/// the file's first bytes for the JS classifier. Writes `destPath` directly. The caller supplies
...
public enum StreamCopier {
  public struct CopyResult: Equatable {
    ...
    /// The file's first bytes (up to 32), captured off the first chunk. Type
    /// classification is the JS side's job; native only saves it a read.
    public let headBytes: Data

    public init(size: Int64, sha256Hex: String, headBytes: Data) {
      self.size = size
  ...
  public static let headByteCount = 32
  ...
  public static func copy(
    ...
    while true {
      ...
      if head.count < Self.headByteCount {
        head.append(chunk.prefix(Self.headByteCount - head.count))
      }
      do {
    ...
    try? output?.close()
    return CopyResult(size: size, sha256Hex: sink.finalizeHex(), headBytes: head)
  }
