...
final class StreamCopierTests: XCTestCase {
  ...
  func testWritesDestPathDirectlyAndReturnsSizeHashHeadBytesFromOneRead() throws {
    ...
    XCTAssertEqual(result.headBytes, payload.prefix(StreamCopier.headByteCount))
    var sink = Sha256Sink()
    sink.update(payload)
  ...
  func testAFileShorterThanTheHeadWindowReturnsItsWholeContentAsHeadBytes() throws {
    let payload = Data([0x25, 0x50, 0x44, 0x46, 0x2D])
    let source = try write("tiny.pdf", payload)
    let dest = dir.appendingPathComponent("tiny.token.tmp").path
    let result = try StreamCopier.copy(sourcePath: source, destPath: dest)
    XCTAssertEqual(result.headBytes, payload)
  }

  func testAnEmptyFileReturnsEmptyHeadBytes() throws {
    let source = try write("empty.bin", Data())
    let dest = dir.appendingPathComponent("empty.token.tmp").path
    let result = try StreamCopier.copy(sourcePath: source, destPath: dest)
    XCTAssertEqual(result.size, 0)
    XCTAssertTrue(result.headBytes.isEmpty)
  }
