...
final class DocumentPickerPresenter: NSObject, UIDocumentPickerDelegate {
  ...
  func documentPicker(_ controller: UIDocumentPickerViewController, didPickDocumentsAt urls: [URL]) {
    Self.active = nil
    // Metadata and the bookmark, under a momentary scope; never read file
    // bodies here (a 30k pick must return without moving bytes). The bookmark
    // MUST be created from these delivered URL objects: the security scope is
    // a property of the instance, so a URL rebuilt later from the uri string
    // has no access and mints a bookmark that can never resolve.
    let files: [[String: Any]] = urls.map { url in
      ...
      do {
        file["ref"] = try BookmarkEngine.create(url: url)
      } catch let error as CodedError {
        file["refError"] = error.code
      } catch {
        file["refError"] = "not-persistable"
      }
