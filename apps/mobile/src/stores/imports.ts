...
/** In-flight import files destined for one folder, or across every import
 * when `directoryId` is null (the root view's banner); drives the banner. */
export function useInFlightCountForDirectory(
  directoryId: string | null,
  config?: SWRConfiguration,
) {
  const key = app().caches.imports.key('inFlightForDir')
  return useSWR(
    [...key, directoryId ?? ''],
    () => app().imports.countInFlight(directoryId ?? undefined),
    config,
  )
}
