...
export async function capturePickedAssets(
  ...
  if (opts.openInPlace) {
    // iOS open-in-place picks carry their bookmark already: the pick delegate
    // mints it while the picker's security-scoped URL objects exist. A
    // bookmark created here, from the uri string, would have no scope and
    // could never resolve, so a pick without a ref stays ephemeral.
    return picked.map((a) => {
      if (a.sourceRef) return { ...a, sourceKind: 'bookmark' as const }
      logger.warn('importCapture', 'pick_missing_ref', { name: a.name })
      return a
    })
  }

  const remaining = await SourceRefs.grantBudgetRemaining()
  const room = Math.max(0, remaining - IMPORT_GRANT_RESERVE)
