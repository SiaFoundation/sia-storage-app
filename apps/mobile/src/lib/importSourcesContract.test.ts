...
describe('import-sources contract', () => {
  ...
  it('pickFiles calls native on Android too', async () => {
    const refs = { pickFiles: jest.fn(async () => []) }
    const mod = loadModule({ refs }, 'android')

    await expect(mod.pickFiles()).resolves.toEqual([])
    expect(refs.pickFiles).toHaveBeenCalled()
  })

  it('pickMedia maps a native cancelled rejection to an empty array', async () => {
    const refs = {
    ...
    const mod = loadModule({ refs })
    await expect(mod.pickMedia()).resolves.toEqual([])
  })
