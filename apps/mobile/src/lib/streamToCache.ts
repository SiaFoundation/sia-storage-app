...
import { getOrCreateTempDownloadFile, removeTempDownloadFile } from '../stores/tempFs'
...
export async function streamToCache(params: {
  ...
  const tempInfo = { ...file, localId: null }
  const targetFile = await getOrCreateTempDownloadFile(tempInfo)
  ...
  try {
    try {
      ...
      // Nested so dl.cancel() still runs if close() throws.
      try {
    ...
    // An aborted or failed download leaves a partial temp behind; a successful
    // run consumes it in onAfterClose. Nothing else sweeps the cache directory,
    // so remove it here.
    await removeTempDownloadFile(tempInfo)
    throw e
  }
}
