...
import { buildPhotoCandidateRows, importAssets, importMediaAssets } from './assetImports'
...
describe('importMediaAssets for iOS photo picks', () => {
  it('stages accessible picks as pending media-kind rows in a sealed picker import', async () => {
    const { importId } = await importMediaAssets(
      [pick({ id: 'ph-1', name: 'IMG_1.heic', sourceUri: undefined })],
      [],
    )
    const imp = await app().imports.get(importId!)
    expect(imp).toMatchObject({ source: 'picker', sealed: 1, dedupByHash: 0, expectedCount: 1 })
    const rows = await app().imports.files(importId!)
  ...
  it('denied picks land as terminal unavailable rows with the permission-denied reason', async () => {
    const { importId } = await importMediaAssets(
      [pick({ id: 'ph-ok', sourceUri: undefined })],
      [pick({ id: 'ph-denied', name: 'hidden.jpg', sourceUri: undefined })],
    )
    const rows = await app().imports.files(importId!)
    expect(rows).toHaveLength(2)
    expect(rows.find((r) => r.mediaAssetId === 'ph-denied')).toMatchObject({
      state: 'unavailable',
      reason: 'permission-denied',
      sourceKind: 'media',
    })
    const imp = await app().imports.get(importId!)
    expect(imp!.expectedCount).toBe(2)
  })

  it('re-picking an already-imported asset dedups upfront and stages nothing', async () => {
    const first = await importMediaAssets([pick({ id: 'ph-dup', sourceUri: undefined })], [])
    expect(first.importId).toBeTruthy()
    const second = await importMediaAssets([pick({ id: 'ph-dup', sourceUri: undefined })], [])
