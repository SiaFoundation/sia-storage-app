...
//   placeholders instantly, background copy, scanner finalizes. No dedup.
//
// syncAssets — Background recent photo sync (syncNewPhotos). Inline
//   copy+hash in small batches. Dedup by localId and content hash.
//
// catalogAssets — Archive library walk (syncPhotosArchive). Bulk
//   placeholders via INSERT OR IGNORE. No copy, no hash. Scanner
//   handles via tier-2 backpressure. 500 files/tick.
 ...
 * content hashes inline in small batches. Dedup by localId and content
 * hash prevents re-importing files already tracked or synced from another
 * device.
 */
 ...
 * import scanner handles those via tier-2 processing with backpressure.
 */
