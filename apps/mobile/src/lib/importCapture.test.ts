...
describe('capturePickedAssets', () => {
  ...
  it('open-in-place picks use the delegate-minted ref and make no native calls', async () => {
    // The bookmark arrives with the pick: only the picker's own URL objects
    // carry the security scope, so a bookmark created here could not resolve.
    const withRef = { ...pick('file:///doc.pdf'), sourceRef: 'ios-bm:AAAA' }
    const tagged = await capturePickedAssets([withRef], { openInPlace: true })
    expect(jest.mocked(grantBudgetRemaining)).not.toHaveBeenCalled()
    expect(jest.mocked(createFileBookmarks)).not.toHaveBeenCalled()
    expect(tagged[0]).toMatchObject({ sourceKind: 'bookmark', sourceRef: 'ios-bm:AAAA' })
  })

  it('an open-in-place pick without a ref stays ephemeral', async () => {
    const tagged = await capturePickedAssets([pick('file:///doc.pdf')], { openInPlace: true })
    expect(jest.mocked(createFileBookmarks)).not.toHaveBeenCalled()
    expect(tagged[0].sourceKind).toBeUndefined()
  })
