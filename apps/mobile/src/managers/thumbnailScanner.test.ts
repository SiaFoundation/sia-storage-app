    ...
    for (let i = 0; i < 10; i++) {
      ...
      for (const size of ThumbSizes) {
        await app().files.create({
          ...
          kind: 'thumb',
          size: 100,
          hash: `sha256:${id}-thumb-hash-${size}`,
          createdAt: now - 100 - i,
          updatedAt: now - 100 - i,
          addedAt: now - 100 - i,
          mediaAssetId: null,
  ...
  it('skips files whose type is not in the adapter allowlist (camera RAW, JXL, etc.)', async () => {
    ...
    for (let i = 0; i < unsupportedTypes.length; i++) {
      await app().files.create({
        ...
        kind: 'file',
        size: 1000,
        hash: `sha256:unsupported-hash-${i}`,
        createdAt: now - i,
        updatedAt: now - i,
        addedAt: now - i,
