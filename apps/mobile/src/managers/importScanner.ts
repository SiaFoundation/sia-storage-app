...
import { MAGIC_BYTES_LENGTH } from '@siastorage/core/lib/detectMimeType'
...
import { publishImportPacing } from '../stores/importPacing'
...
function mapResolveError(e: unknown): ResolveSourceResult {
  ...
  logger.warn('importScanner', 'resolve_failed', {
    code: code ?? 'none',
...
function ensureInitialized(): void {
  ...
  scanner.initialize(
    ...
    (uri) => readFileBytes(uri, MAGIC_BYTES_LENGTH),
    resolveSource,
    releaseSourceGrant,
  )
}
...
export async function runImportScanner(signal?: AbortSignal): Promise<ImportScannerResult> {
  ...
  try {
    ...
    // Per-ref best-effort: one rejecting close would leak the rest.
    for (const dirRef of openedDirScopes) {
      try {
        await SourceRefs.stopAccessDir(dirRef)
      } catch (e) {
        logger.debug('importScanner', 'dir_scope_close_failed', { error: e as Error })
      }
    }
...
async function run(signal: AbortSignal): Promise<number | undefined> {
  ...
  publishImportPacing({
    at: Date.now(),
    cause: result.pressure.cause,
    freeBytes: result.pressure.freeBytes,
    pendingLocalBytes: result.pressure.pendingLocalBytes,
