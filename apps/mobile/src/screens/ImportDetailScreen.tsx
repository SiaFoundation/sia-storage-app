...
function ImportDetailContent({
  ...
  const destination = useMemo(() => {
    const dir = imp?.directoryId ? directories?.find((d) => d.id === imp.directoryId) : null
    return dir ? { id: dir.id, name: dir.name, path: dir.path } : null
  }, [directories, imp?.directoryId])
...
function ImportDetailHeader({
  ...
  destination: { id: string; name: string; path: string } | null
  tags: string[]
  now: number
    ...
    <View style={styles.header}>
          ...
          <InsetGroupValueRow
            ...
            valueSlot={
              <Pressable
                ...
                onPress={() => {
                  if (destination) navigateToDirectory(destination)
                }}
                style={({ pressed }) => [
                  styles.destination,
              ...
              >
                <FolderIcon color={palette.gray[400]} size={14} />
      ...
      {imp && summary && importing ? (
        <View style={styles.progressBlock}>
          ...
          {explainer ? <Text style={styles.progressLabel}>{explainer}</Text> : null}
        </View>
      ) : null}
