...
describe('FS Cache Eviction', () => {
  ...
  it('an evicted file stops reading as downloaded', async () => {
    const [file] = await app.addFiles(generateTestFiles(1, { startId: 1, sizeBytes: 500 }))
    await app.waitForNoActiveUploads()
    // Deleting the fsMeta row is not enough: a per-file cache and a flag on
    // each list row both answer "is this on the device" for the UI.
    const clearUri = jest.spyOn(app.app.caches.fsFileUri, 'set')
    const refreshList = jest.spyOn(app.app.caches.library, 'invalidateAll')

    const result = await runCacheEviction(app.app, EVICT_ALL_BACKED_UP)

    expect(result!.evictedFileIds).toContain(file.id)
    expect(clearUri).toHaveBeenCalledWith(null, file.id)
