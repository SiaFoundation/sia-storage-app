...
use chrono::Utc;
use rusqlite::{
    Connection, OptionalExtension, named_params, params, params_from_iter, types::Value,
};
use tracing::debug;
...
use crate::types::patch::FieldUpdate;
...
/// Predicate shared by the lost-file reads and delete: lostReason set OR (non-empty hash + no
/// objects on this indexer + no local fs entry). An empty-hash row is never lost via the second
/// rule. Binds the indexer by the named `:indexer_url` so a caller can splice this into a larger
/// query without tracking where its `?` would fall in the positional bind order.
const LOST_PREDICATE_SQL: &str = r"(
       f.lostReason IS NOT NULL
       OR (NOT EXISTS (SELECT 1 FROM objects s WHERE s.fileId = f.id AND s.indexerURL = :indexer_url)
...
/// Builds the domain `FileRecord` from a row and its object refs, keyed by indexerURL: callers ask
/// "the object on indexer X" and "which indexers hold this file", both one-per-indexer. A file with
/// two objects on one indexer is an edge case that collapses to whichever the query yields last,
/// which is harmless here, presence reads the same either way, and both rows still delete together
/// by fileId. The refs are slim stubs (no slabs); use `read_file_with_objects` for the full objects.
pub(in crate::db) fn transform_row(
    row: FileRecordRow,
    objects: Option<Vec<LocalObjectRef>>,
) -> FileRecord {
...
/// Which side of the hash-emptiness predicate to keep. An empty hash marks an import
/// placeholder whose content is not yet hashed.
#[derive(Debug, Clone, Copy)]
pub enum HashFilter {
    /// Only rows with an empty hash.
    Empty,
    /// Only rows with a non-empty hash.
    NotEmpty,
}
...
pub struct FileQueryOpts {
    ...
    /// Filter on hash emptiness; `None` applies no hash filter.
    pub hash: Option<HashFilter>,
...
/// Build the WHERE clause and its bound params for a `files` query from `opts`, over `table_alias`.
/// Shared by the row query and the aggregates; ordering and limiting belong to the row query alone.
fn build_file_where(opts: &FileQueryOpts, table_alias: &str) -> (String, Vec<Value>) {
    ...
    match opts.hash {
        Some(HashFilter::Empty) => where_clauses.push(format!("{table_alias}.hash = ''")),
        Some(HashFilter::NotEmpty) => where_clauses.push(format!("{table_alias}.hash != ''")),
        None => {}
    }
...
fn build_file_records_query(
    ...
    let (where_sql, mut params) = build_file_where(opts, table_alias);
...
/// Run a single-group aggregate over `files` with the given SELECT `projection`. Only the WHERE
/// filters apply; the query returns one row, so there is nothing to order or limit.
fn aggregate_files_query<T>(
    ...
    let (where_sql, params) = build_file_where(opts, "files");
    let q = format!("SELECT {projection} FROM files {where_sql}");
...
/// A partial update to one file row, keyed by `id`. Every field is a [`FieldUpdate`]: `Unchanged`
/// leaves the column, `Changed(v)` writes v. Nullable columns carry `FieldUpdate<Option<_>>`, so
/// `Changed(None)` writes SQL NULL.
#[derive(Debug, Clone, Default)]
pub struct FileUpdate {
    pub id: String,
    pub name: FieldUpdate<String>,
    pub type_: FieldUpdate<String>,
    pub kind: FieldUpdate<FileKind>,
    pub size: FieldUpdate<u64>,
    pub hash: FieldUpdate<String>,
    pub created_at: FieldUpdate<i64>,
    pub updated_at: FieldUpdate<i64>,
    pub thumb_for_id: FieldUpdate<Option<String>>,
    pub thumb_size: FieldUpdate<Option<ThumbSize>>,
    pub local_id: FieldUpdate<Option<String>>,
    pub trashed_at: FieldUpdate<Option<i64>>,
    pub deleted_at: FieldUpdate<Option<i64>>,
    pub lost_reason: FieldUpdate<Option<String>>,
}
...
pub(in crate::db) fn recalculate_current_for_group_stmt(
    ...
    // The trashed/deleted guards mean a trashed row keeps a stale `current = 1`; harmless,
    // every view filters trashed rows, and the recalc on restore reconverges the group.
    conn.execute(
...
/// Update only the `Changed` fields of `update`. Sets `updatedAt = now` itself unless
/// `include_updated_at` (then it uses the supplied value), then flags the file's objects for
/// sync-up. Unless `current_recalc` is `Skip`, anything that reorders a group's newest row
/// (a name/trashedAt/deletedAt change, or any `updatedAt` write, including the auto-bump)
...
pub(in crate::db) fn update_file_stmt(
    ...
    if let FieldUpdate::Changed(name) = &update.name {
        assignments.push(("name", Value::Text(name.clone())));
    }
    if let FieldUpdate::Changed(t) = &update.type_ {
        assignments.push(("type", Value::Text(t.clone())));
    }
    if let FieldUpdate::Changed(k) = &update.kind {
        assignments.push(("kind", Value::Text(file_kind_str(*k).into())));
    }
    if let FieldUpdate::Changed(s) = &update.size {
        assignments.push(("size", Value::Integer(*s as i64)));
    }
    if let FieldUpdate::Changed(h) = &update.hash {
        assignments.push(("hash", Value::Text(h.clone())));
    }
    if let FieldUpdate::Changed(c) = &update.created_at {
        assignments.push(("createdAt", Value::Integer(*c)));
    }
    if let FieldUpdate::Changed(t) = &update.thumb_for_id {
        assignments.push(("thumbForId", t.clone().into()));
    }
    if let FieldUpdate::Changed(t) = &update.thumb_size {
        assignments.push(("thumbSize", (*t).map(i64::from).into()));
    }
    if let FieldUpdate::Changed(l) = &update.local_id {
        assignments.push(("localId", l.clone().into()));
    }
    if let FieldUpdate::Changed(t) = &update.trashed_at {
        assignments.push(("trashedAt", (*t).into()));
    }
    if let FieldUpdate::Changed(d) = &update.deleted_at {
        assignments.push(("deletedAt", (*d).into()));
    }
    if let FieldUpdate::Changed(r) = &update.lost_reason {
        assignments.push(("lostReason", r.clone().into()));
    }
    if options.include_updated_at
        && let FieldUpdate::Changed(u) = &update.updated_at
    {
    ...
    // updatedAt is written on the `!include_updated_at` path (auto-bumped to now) and on the
    // include path only when the caller supplied one; either can make a non-current row the
    // group's newest, so recalc keys off whether updatedAt was actually written, not just
    // whether the caller passed one.
    let updated_at_written = !options.include_updated_at || update.updated_at.is_changed();
    let needs_recalc = options.current_recalc == CurrentRecalc::Run
        && (update.name.is_changed()
            || update.trashed_at.is_changed()
            || update.deleted_at.is_changed()
            || updated_at_written);
    ...
    if let Some(g) = old_group {
        recalculate_current_for_group_stmt(conn, &g.name, g.directory_id.as_deref())?;
        if let FieldUpdate::Changed(new_name) = &update.name
            && new_name != &g.name
        {
...
pub(in crate::db) fn upsert_many_files_stmt(
    ...
    // Under Run, capture the groups existing rows sit in before the upsert rewrites their
    // names, like update_file_stmt: the post-upsert recalc only sees each row's new group, so
    // a rename that moves the current row out would otherwise leave its old group with none.
    let pre_upsert_groups = if current_recalc == CurrentRecalc::Run {
        let ids: Vec<String> = records.iter().map(|r| r.id.clone()).collect();
        query_name_dir_groups(conn, &ids)?
    } else {
        Vec::new()
    };
    let mut stmt = conn.prepare(
    ...
    recalculate_current_for_groups_stmt(conn, &pre_upsert_groups)
}
...
impl Db {
    ...
    pub async fn count_lost_files(&self, indexer_url: String) -> Result<u64, DbError> {
        self.transaction(move |c| {
            ...
            Ok(
                c.query_row(&sql, named_params! { ":indexer_url": indexer_url }, |r| {
                    r.get(0)
                })?,
            )
        })
    ...
    pub async fn query_lost_files(
        ...
        self.transaction(move |c| {
            ...
            let out = stmt
                .query_map(
                    named_params! { ":indexer_url": indexer_url },
                    file_record_row_from_db_row,
                )?
                .collect::<rusqlite::Result<Vec<FileRecordRow>>>()?;
            Ok(out)
    ...
    pub async fn read_file_with_objects(
        ...
        self.transaction(move |c| {
            ...
            // Keyed by indexerURL; a same-indexer duplicate collapses arbitrarily and harmlessly
            // (see transform_row).
            let objects = objects
                .into_iter()
    ...
    /// Hard-delete every active "lost" file (and its thumbnails) that has no object on
    /// ANY indexer, returning the count deleted. A file is lost against `indexer_url` when it has
    /// no object there, but deleting the row cascades its object rows away via the objects foreign
    /// key, so one still hosted on a different indexer must be left alone; the no-object-anywhere
    /// guard is what prevents that over-delete. Includes old versions, not just the current one.
    pub async fn delete_lost_files_and_thumbnails(
        ...
        self.transaction(move |c| {
            ...
            let ids: Vec<String> = {
                let mut stmt = c.prepare(&sql)?;
                stmt.query_map(named_params! { ":indexer_url": indexer_url }, |r| r.get(0))?
                    .collect::<rusqlite::Result<Vec<String>>>()?
            };
    ...
    /// Update only the `Changed` fields of `update`, then flag the file's objects for sync-up.
    /// Unless `current_recalc` is `Skip`, a change that reorders a group's newest row recomputes
    /// `current` for the affected groups.
...
mod tests {
    ...
    // A full valid `file`-kind row: name and updatedAt from the caller, hash derived from the
    // id so distinct rows never share content identity.
    fn version(id: &str, name: &str, updated_at: i64) -> FileRecordRow {
        FileRecordRow {
            id: id.into(),
            name: name.into(),
    ...
    async fn count_unuploaded_files_excludes_files_with_objects() {
        ...
        put_file(&db, "f2", "b", None, "file", "h2", 1, 2, None).await;
        put_object(&db, "f1", "o1", "https://a.com").await;
        // f2 has no object, so counted; f1 has one, so excluded.
    ...
    async fn query_active_file_summaries_returns_active_set() {
        ...
        // A trashed row is excluded by build_record_filter.
    ...
    async fn count_lost_files_counts_lost_files() {
        let db = test_db().await;
        // f1: lostReason set, so lost regardless of objects.
        put_file(
            &db,
            "f1",
            "a",
        ...
        .await;
        // f2: no object, no fs, non-empty hash, so lost.
        put_file(&db, "f2", "b", None, "file", "h2", 1, 2, None).await;
        // f3: has an object on the indexer, so NOT lost.
        put_file(&db, "f3", "c", None, "file", "h3", 1, 3, None).await;
        put_object(&db, "f3", "o3", "https://a.com").await;
        // f4: empty hash, no object, no fs, so NOT lost (hash != '' fails).
        put_file(&db, "f4", "d", None, "file", "", 1, 4, None).await;
        // f5: present locally (fs row), so NOT lost.
    ...
    // A file lost against indexer A (no object there) but still hosted on indexer B must survive
    // A's lost-cleanup: deleting the row would cascade its B object away via the objects foreign
    // key. Only files with no object on any indexer get swept.
    #[tokio::test]
    async fn delete_lost_files_spares_files_hosted_on_another_indexer() {
        let db = test_db().await;
        // f1: no object anywhere, non-empty hash, so genuinely lost and deletable.
        put_file(&db, "f1", "a", None, "file", "h1", 1, 1, None).await;
        // f2: no object on A but one on B, so lost against A yet must not be deleted.
        put_file(&db, "f2", "b", None, "file", "h2", 1, 2, None).await;
        put_object(&db, "f2", "o2", "https://b.com").await;

        let deleted = db
            .delete_lost_files_and_thumbnails("https://a.com".to_string())
            .await
            .unwrap();
        assert_eq!(deleted, 1);
        ...
        let remaining: Vec<String> = db
            ...
            .await
            .unwrap();
        assert_eq!(remaining, vec!["f2".to_string()]);
    }

    // The aggregate is WHERE-only: it counts the active set and ignores opts.order/limit. The
    // limit=0 case is the one that would otherwise break, a LIMIT 0 makes query_row find no row
    // and error.
    #[tokio::test]
    async fn count_files_counts_active_set_and_ignores_limit() {
        let db = test_db().await;
        put_file(&db, "f1", "a", None, "file", "h1", 1, 1, None).await;
        put_file(&db, "f2", "b", None, "file", "h2", 1, 2, None).await;
        assert_eq!(db.count_files(FileQueryOpts::default()).await.unwrap(), 2);
        assert_eq!(
            db.count_files(FileQueryOpts {
                limit: Some(0),
                ..Default::default()
            })
            .await
    ...
    async fn update_file_name_writes_name_sort_key_and_bumps_updated_at() {
        ...
        db.update_file(
            FileUpdate {
                id: "f1".into(),
                name: FieldUpdate::Changed("File 10.txt".into()),
                ..Default::default()
            },
    ...
    async fn update_file_include_updated_at_uses_supplied_value() {
        ...
        db.update_file(
            FileUpdate {
                id: "f1".into(),
                name: FieldUpdate::Changed("b.txt".into()),
                updated_at: FieldUpdate::Changed(12345),
                ..Default::default()
            },
            UpdateFileOptions {
    ...
    async fn insert_file_recalculates_current_to_newest_version() {
        let db = test_db().await;
        db.insert_file(version("v1", "a.txt", 100), InsertFileOptions::default())
            .await
            .unwrap();
        db.insert_file(version("v2", "a.txt", 200), InsertFileOptions::default())
            .await
            .unwrap();
    ...
    // Currency is defined as the newest updatedAt in the group, and an update with default options
    // auto-bumps updatedAt to now, so touching any row makes it the newest and hands it current.
    // The recalc keys off that auto-bump, not only a caller-supplied updatedAt. The UX never edits
    // an old version, so this promotion doesn't arise in practice; the test pins the underlying
    // rule (current tracks the timestamp) and that the recalc stays consistent with it.
    #[tokio::test]
    async fn update_bumps_updated_at_so_the_edited_row_becomes_current() {
        let db = test_db().await;
        db.insert_file(version("v1", "a.txt", 100), InsertFileOptions::default())
            .await
            .unwrap();
        ...
        assert_eq!(current_flag(&db, "v2").await, 0);
    }
    ...
    async fn upsert_many_files_preserves_unlisted_columns() {
        ...
        let incoming = FileRecordRow {
            hash: "h_new".into(),
            local_id: Some("L2".into()),
            deleted_at: Some(99),
            added_at: 123,
            ..version("f1", "new", 1000)
        };
    ...
    // A Run-mode upsert that renames the current version out of a two-version group must
    // recompute the group it left, not just the one it entered, so the remaining version takes
    // current there. Whole-stack renames (rename_all_file_versions) empty the old group, making
    // that recalc a no-op; this locks the contract for a caller that moves only part of a group.
    #[tokio::test]
    async fn upsert_run_renaming_current_away_recomputes_the_old_group() {
        let db = test_db().await;
        db.insert_file(version("v1", "a.txt", 100), InsertFileOptions::default())
            .await
            .unwrap();
        ...
        db.upsert_many_files(vec![version("v2", "b.txt", 300)], CurrentRecalc::Run)
            .await
            .unwrap();
