...
/// `col` starts with the `:prefix` named bind (the parent path plus a trailing `/`). Named so
/// the fragment splices into any host statement without depending on positional bind order.
fn starts_with_param(col: &str) -> String {
    format!("substr({col}, 1, length(:prefix)) = :prefix")
}
...
/// Which directories a counts listing returns.
enum DirectoryScope<'a> {
    All,
    /// Immediate children of the given parent path; `None` is the root level.
    ChildrenOf(Option<&'a str>),
}

/// Every directory in `scope` with its recursive file count and direct-subdirectory count,
/// ordered by natural-sort name. One pass: a single GROUP BY over files gives each directory's
/// direct count, rolled up its ancestor chain in memory. A correlated per-row subtree subquery
/// re-scans for every listed row, which is seconds-versus-milliseconds at a 100k-file library.
fn query_directories_with_counts_stmt(
    conn: &Connection,
    scope: DirectoryScope<'_>,
) -> Result<Vec<DirectoryWithCount>, DbError> {
    let dirs: Vec<Directory> = {
        let mut stmt =
            conn.prepare("SELECT id, path, createdAt FROM directories ORDER BY nameSortKey")?;
        stmt.query_map([], directory_from_db_row)?
            .collect::<rusqlite::Result<Vec<_>>>()?
    };
    let active = build_record_filter("f", BuildRecordFilterOpts::default());
    let direct: std::collections::HashMap<String, u64> = {
        let q = format!(
            "SELECT f.directoryId, COUNT(*) FROM files f
              WHERE f.directoryId IS NOT NULL AND {active}
    ...
    let mut file_counts: std::collections::HashMap<&str, u64> = std::collections::HashMap::new();
    let mut subdir_counts: std::collections::HashMap<&str, u64> = std::collections::HashMap::new();
    for d in &dirs {
        let count = direct.get(&d.id).copied().unwrap_or(0);
        if count > 0 {
            // The directory's own subtree count, plus every ancestor's.
            *file_counts.entry(d.path.as_str()).or_default() += count;
            for (i, _) in d.path.match_indices('/') {
                *file_counts.entry(&d.path[..i]).or_default() += count;
            }
        }
        if let Some(i) = d.path.rfind('/') {
            *subdir_counts.entry(&d.path[..i]).or_default() += 1;
        }
    }
    ...
    Ok(dirs
        ...
        .filter(|d| match scope {
            DirectoryScope::All => true,
            DirectoryScope::ChildrenOf(None) => !d.path.contains('/'),
            DirectoryScope::ChildrenOf(Some(parent)) => {
                d.path.rfind('/').is_some_and(|i| &d.path[..i] == parent)
            }
        })
        .map(|d| DirectoryWithCount {
            directory: d.clone(),
            file_count: file_counts.get(d.path.as_str()).copied().unwrap_or(0),
            subdirectory_count: subdir_counts.get(d.path.as_str()).copied().unwrap_or(0),
        })
        .collect())
}
...
fn query_subtree_directory_ids(conn: &Connection, path: &str) -> Result<Vec<String>, DbError> {
    let mut stmt = conn.prepare(&format!(
        "SELECT id FROM directories WHERE path = :path OR {}",
        starts_with_param("path")
    ))?;
    let out = stmt
        .query_map(
            rusqlite::named_params! { ":path": path, ":prefix": format!("{path}/") },
            |r| r.get(0),
        )?
...
fn rebase_directory_tree(
    ...
    let descendants: Vec<(String, String)> = {
        ...
        stmt.query_map(rusqlite::named_params! { ":prefix": old_prefix }, |r| {
            Ok((r.get(0)?, r.get(1)?))
        })?
        .collect::<rusqlite::Result<Vec<(String, String)>>>()?
    };
    ...
    // or move never reaches other devices. RETURNING hands back the bumped ids for
    // the flagging in the same scan.
    let subtree_file_ids: Vec<String> = {
        let mut stmt = conn.prepare(&format!(
            r"UPDATE files SET updatedAt = :now WHERE directoryId IN (
                SELECT id FROM directories WHERE path = :path OR {}
              ) RETURNING id",
            starts_with_param("path")
        ))?;
...
impl Db {
    pub async fn insert_directory(
        ...
        self.transaction(move |c| {
            ...
            // The path's UNIQUE constraint detects the duplicate in the same statement;
            // zero rows changed means another directory already holds this path.
            ...
            if inserted == 0 {
                return Err(DirectoryError::AlreadyExists(full_path));
            }
            Ok(dir)
        })
        .await
    ...
    pub async fn get_or_create_directory(
        ...
        self.transaction(move |c| get_or_create_directory_stmt(c, &name, parent_path.as_deref()))
            .await
    }
    ...
    pub async fn get_or_create_directory_at_path(
        ...
        self.transaction(move |c| get_or_create_directory_at_path_stmt(c, &dir_path))
            .await
    }
    ...
    pub async fn query_directory_children(
        ...
        self.transaction(move |c| {
            query_directories_with_counts_stmt(
                c,
                DirectoryScope::ChildrenOf(parent_path.as_deref()),
            )
        })
        .await
    }
    ...
    pub async fn query_all_directories_with_counts(
        ...
        self.transaction(move |c| query_directories_with_counts_stmt(c, DirectoryScope::All))
            .await
    }
    ...
    pub async fn sync_directory_from_metadata(
        ...
        self.transaction(move |c| {
            let dir = get_or_create_directory_at_path_stmt(c, &p)?;
    ...
    pub async fn sync_directories_from_metadata(
        ...
        self.transaction(move |c| sync_directories_from_metadata_stmt(c, &entries))
            .await
    }
    ...
    pub async fn delete_directory(&self, id: String) -> Result<(), DbError> {
        self.transaction(move |c| {
            ...
            // reparent syncs up; RETURNING hands back the orphaned ids for the flagging.
        ...
        self.transaction(move |c| {
            ...
            let query = format!(
                r"SELECT f.id FROM files f
                  ...
                  WHERE (d.path = :path OR {})
                    AND f.kind = 'file' AND {} LIMIT :limit",
            ...
            // vanish from every browse view. The bump + flag sync the reparent; RETURNING
            // hands back the detached ids for the flagging.
            let detached: Vec<String> = {
                let mut stmt = c.prepare(&format!(
                    r"UPDATE files SET directoryId = NULL, updatedAt = :now
                      WHERE directoryId IN (SELECT id FROM directories WHERE path = :path OR {})
                      RETURNING id",
                    starts_with_param("path")
                ))?;
                stmt.query_map(
            ...
            c.execute(
                &format!(
                    "DELETE FROM directories WHERE path = :path OR {}",
                    starts_with_param("path")
                ),
                rusqlite::named_params! { ":path": dir.path, ":prefix": prefix },
            )?;
    ...
    pub async fn rename_directory(
        ...
        self.transaction(move |c| {
            let Some(dir) = query_directory_by_id_stmt(c, &dir_id)? else {
            ...
            if query_directory_id_at_path_excluding(c, &new_path, &dir_id)?.is_some() {
                return Err(DirectoryError::AlreadyExists(new_path));
            }
        ...
        self.transaction(move |c| {
            ...
            if let Some(p) = new_parent_path.as_deref() {
                if p == dir.path || p.starts_with(&format!("{}/", dir.path)) {
                    return Err(DirectoryError::MoveIntoSelf);
                }
            ...
            if query_directory_id_at_path_excluding(c, &new_path, &dir_id)?.is_some() {
                return Err(DirectoryError::AlreadyExistsAtDest(leaf_name));
            }
            rebase_directory_tree(c, &dir_id, &dir.path, &new_path)?;
            Ok(())
        })
...
mod tests {
    ...
    async fn create_test_file(db: &Db, id: &str) {
        db.insert_file(row(id), InsertFileOptions::default())
            .await
            .expect("insert test file");
    }

    async fn mkdir(db: &Db, name: &str, parent: Option<&str>) -> Directory {
        db.insert_directory(name.to_string(), parent.map(str::to_string))
            .await
            .expect("insert directory")
    }

    async fn move_file(db: &Db, file_id: &str, dir_id: &str) {
        db.move_file_to_directory(file_id.to_string(), Some(dir_id.to_string()))
            .await
            .expect("move file");
    }

    async fn trash_row(db: &Db, id: &str) {
        let id = id.to_string();
    ...
    async fn insert_named_file(db: &Db, id: &str, name: &str, kind: FileKind) {
        let row = match kind {
            FileKind::Thumb => FileRecordRow {
                name: name.into(),
                kind: FileKind::Thumb,
                thumb_for_id: Some("orig".into()),
                thumb_size: Some(ThumbSize::S512),
                ..row(id)
            },
            FileKind::File => FileRecordRow {
                name: name.into(),
                ..row(id)
            },
        };
    ...
    async fn insert_directory_creates_a_root_directory() {
        ...
        let dir = mkdir(&db, "Photos", None).await;
    ...
    async fn insert_directory_creates_a_nested_directory() {
        let db = test_db().await;
        mkdir(&db, "Photos", None).await;
        let dir = mkdir(&db, "Vacation", Some("Photos")).await;
    ...
    async fn paths_are_case_sensitive() {
        let db = test_db().await;
        let lower = mkdir(&db, "photos", None).await;
        let upper = mkdir(&db, "Photos", None).await;
    ...
    async fn query_directory_children_returns_root_children_with_counts() {
        let db = test_db().await;
        let photos = mkdir(&db, "Photos", None).await;
        mkdir(&db, "Vacation", Some("Photos")).await;
        mkdir(&db, "Videos", None).await;
        create_test_file(&db, "f1").await;
        move_file(&db, "f1", &photos.id).await;
    ...
    async fn query_directory_children_counts_files_recursively_across_descendants() {
        let db = test_db().await;
        mkdir(&db, "Photos", None).await;
        mkdir(&db, "Trips", Some("Photos")).await;
        let italy = mkdir(&db, "Italy", Some("Photos/Trips")).await;
        create_test_file(&db, "f1").await;
        move_file(&db, "f1", &italy.id).await;
    ...
    async fn query_directory_children_treats_underscore_literally() {
        let db = test_db().await;
        mkdir(&db, "a_b", None).await;
        mkdir(&db, "axb", None).await;
        mkdir(&db, "child", Some("a_b")).await;
        mkdir(&db, "kid", Some("axb")).await;
    ...
    async fn query_directory_children_treats_percent_literally() {
        let db = test_db().await;
        mkdir(&db, "50% off", None).await;
        mkdir(&db, "child", Some("50% off")).await;
        mkdir(&db, "50X off", None).await;
        mkdir(&db, "other", Some("50X off")).await;
    ...
    async fn query_all_directories_with_counts_excludes_trashed_files() {
        ...
        move_file(&db, "f1", &dir.id).await;
        move_file(&db, "f2", &dir.id).await;
        trash_row(&db, "f2").await;
    ...
    async fn query_directory_path_for_file_returns_nested_path() {
        let db = test_db().await;
        mkdir(&db, "Photos", None).await;
        let vacation = mkdir(&db, "Vacation", Some("Photos")).await;
        create_test_file(&db, "f1").await;
        move_file(&db, "f1", &vacation.id).await;
    ...
    async fn rename_directory_renames_and_updates_descendant_paths() {
        ...
        let dir = mkdir(&db, "Photos", None).await;
        mkdir(&db, "Vacation", Some("Photos")).await;
        mkdir(&db, "2025", Some("Photos/Vacation")).await;
    ...
    async fn delete_directory_orphans_subtree_files_to_root() {
        let db = test_db().await;
        let dir = mkdir(&db, "Photos", None).await;
        let sub = mkdir(&db, "A", Some("Photos")).await;
        mkdir(&db, "B", Some("Photos/A")).await;
        create_test_file(&db, "f1").await;
        create_test_file(&db, "f2").await;
        move_file(&db, "f1", &dir.id).await;
        move_file(&db, "f2", &sub.id).await;
    ...
    async fn delete_directory_leaves_siblings_untouched() {
        ...
        let photos = mkdir(&db, "Photos", None).await;
        mkdir(&db, "Videos", None).await;
        let work = mkdir(&db, "Work", Some("Videos")).await;
        create_test_file(&db, "f1").await;
        create_test_file(&db, "f2").await;
        move_file(&db, "f1", &photos.id).await;
        move_file(&db, "f2", &work.id).await;
    ...
    async fn delete_directory_and_trash_files_trashes_every_level_and_deletes_subtree() {
        ...
        let dir = mkdir(&db, "Photos", None).await;
        let vacation = mkdir(&db, "Vacation", Some("Photos")).await;
        let deep = mkdir(&db, "2025", Some("Photos/Vacation")).await;
        create_test_file(&db, "f1").await;
        create_test_file(&db, "f2").await;
        create_test_file(&db, "f3").await;
        move_file(&db, "f1", &dir.id).await;
        move_file(&db, "f2", &vacation.id).await;
        move_file(&db, "f3", &deep.id).await;
    ...
    async fn delete_directory_and_trash_files_handles_more_than_500_files() {
        let db = test_db().await;
        let dir = mkdir(&db, "Big", None).await;
        let n = 1001;
        for i in 0..n {
            let id = format!("big-{i}");
    ...
    async fn delete_directory_and_trash_files_subtree_match_is_literal() {
        let db = test_db().await;
        let a_und_b = mkdir(&db, "a_b", None).await;
        let child = mkdir(&db, "child", Some("a_b")).await;
        mkdir(&db, "axb", None).await;
        let sibling_child = mkdir(&db, "child", Some("axb")).await;
        create_test_file(&db, "f1").await;
        create_test_file(&db, "f2").await;
        create_test_file(&db, "f3").await;
        move_file(&db, "f1", &a_und_b.id).await;
        move_file(&db, "f2", &child.id).await;
        move_file(&db, "f3", &sibling_child.id).await;
    ...
    async fn delete_empty_directories_deletes_an_empty_directory() {
        let db = test_db().await;
        let dir = mkdir(&db, "target", None).await;
        assert_eq!(
    ...
    async fn delete_empty_directories_keeps_directory_with_active_files() {
        let db = test_db().await;
        let dir = mkdir(&db, "full", None).await;
        create_test_file(&db, "f1").await;
        move_file(&db, "f1", &dir.id).await;
        assert_eq!(
            db.delete_empty_directories(vec![dir.id])
    ...
    async fn delete_empty_directories_deletes_when_all_files_trashed() {
        let db = test_db().await;
        let dir = mkdir(&db, "trashed", None).await;
        create_test_file(&db, "f1").await;
        move_file(&db, "f1", &dir.id).await;
        trash_row(&db, "f1").await;
        assert_eq!(
            db.delete_empty_directories(vec![dir.id])
                .await
                .expect("del"),
    ...
    async fn delete_empty_directories_ignores_non_current_versions() {
        let db = test_db().await;
        let dir = mkdir(&db, "versioned", None).await;
        create_test_file(&db, "f1").await;
        move_file(&db, "f1", &dir.id).await;
        db.transaction(|c| {
    ...
    async fn delete_empty_directories_keeps_directory_with_subdirectories() {
        let db = test_db().await;
        let parent = mkdir(&db, "hasChild", None).await;
        mkdir(&db, "sub", Some("hasChild")).await;
        assert_eq!(
    ...
    async fn delete_empty_directories_walks_up_and_deletes_empty_parent() {
        let db = test_db().await;
        mkdir(&db, "outer", None).await;
        let child = mkdir(&db, "inner", Some("outer")).await;
        assert_eq!(
    ...
    async fn delete_empty_directories_stops_at_parent_with_files() {
        let db = test_db().await;
        let parent = mkdir(&db, "parent", None).await;
        let child = mkdir(&db, "child", Some("parent")).await;
        create_test_file(&db, "f1").await;
        move_file(&db, "f1", &parent.id).await;
        assert_eq!(
            db.delete_empty_directories(vec![child.id])
                .await
                .expect("del"),
    ...
    async fn delete_empty_directories_ignores_non_existent_ids() {
        let db = test_db().await;
        let dir = mkdir(&db, "real", None).await;
        let deleted = db
    ...
    async fn move_directory_moves_under_new_parent_with_descendants() {
        let db = test_db().await;
        mkdir(&db, "Work", None).await;
        let reports = mkdir(&db, "Reports", Some("Work")).await;
        mkdir(&db, "Q1", Some("Work/Reports")).await;
        mkdir(&db, "Archive", None).await;
    ...
    async fn move_directory_bumps_updated_at_on_subtree_files() {
        ...
        mkdir(&db, "Work", None).await;
        let reports = mkdir(&db, "Reports", Some("Work")).await;
        let q1 = mkdir(&db, "Q1", Some("Work/Reports")).await;
        mkdir(&db, "Archive", None).await;
        create_test_file(&db, "f1").await;
        create_test_file(&db, "f2").await;
