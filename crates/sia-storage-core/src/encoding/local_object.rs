...
/// A malformed hex field decodes to an empty Vec, so one corrupt row never
/// aborts the whole batch load.
...
mod tests {
    ...
    fn round_trip_via_storage_row_with_populated_slabs() {
        ...
        assert_eq!(back.slabs, original.slabs);
    }

    #[test]
    fn malformed_hex_fields_decode_to_empty_vec_instead_of_failing_the_row() {
        let original = sample();
